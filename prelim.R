## Set working directories  
  setwd("C:/Users/new user/OneDrive/Kaggle/Facial Points Recog")
  data.dir   <- 'C:/Users/new user/OneDrive/Kaggle/Facial Points Recog/data_in/'
...
## Separate out rightmost column ##
  im.train      <- d.train$Image
  d.train$Image <- NULL
  
## Convert to vector
  library(doParallel)
  #register(doParallel) ## gives warning because can't find register function
  im.train <- foreach(im = im.train, .combine=rbind) %dopar% {
    as.integer(unlist(strsplit(im, " ")))
  } ## gives warning because was unable to register doParallel 
...
## How much does nose location vary?
...
## Improve Algo using image patches ##
  ...
  ## Visualize one patch as an example
  ...
  submission_Layout <- read.csv(paste0(data.dir,'idLookupTable.csv'))  
  sub.col.names <- names(submission_Layout)
  submission_Layout$Location <- NULL
  
  library(data.table)
  dt.submission_Layout<- data.table(submission_Layout) ## convert to data table for efficiency
  setkeyv(dt.submission_Layout, c('ImageId', 'FeatureName'))
  
  dt.submission <- data.table(submission) ## convert to data table for efficiency
  setkeyv(dt.submission, c('ImageId', 'FeatureName'))
  
  submission <- merge(dt.submission_Layout, dt.submission, all.x=T, sort=F)
  submission$ImageId <- NULL
  submission$FeatureName <- NULL
  
  write.csv(submission, file=gzfile("submission_means1.csv.gz"), quote=F, row.names=F)
