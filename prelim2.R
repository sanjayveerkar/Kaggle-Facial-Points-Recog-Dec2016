...
#### Code for inserting RMSE calculation starts here ######
...
# colnames(p) <- names(d.train)
  ...
  rmse_stats<-sqrt(mean((d.test-p)^2, na.rm=T))
  rmse_stats
...
write.csv(submission, file=gzfile("submission_patches2.csv.gz"), quote=F, row.names=F)
